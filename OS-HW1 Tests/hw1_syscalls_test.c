...
bool test_enable_policy() {
	// test pid < 0
	ASSERT_TEST(enable_policy(-5, 10, PASSWORD) == -1 && errno == ESRCH);
	// test (hopefully) pid does not exist
	ASSERT_TEST(enable_policy(INT_MAX - 2, 10, PASSWORD) == -1 && errno == ESRCH);
	// test incorrect password
	ASSERT_TEST(enable_policy(getpid(), 10, 234122) == -1 && errno == EINVAL);
	// test negative size
	ASSERT_TEST(enable_policy(getpid(), -2, PASSWORD) == -1 && errno == EINVAL);
	// success
	ASSERT_TEST(enable_policy(getpid(), 10, PASSWORD) == 0);
	// test policy already on
	ASSERT_TEST(enable_policy(getpid(), 10, PASSWORD) == -1 && errno == EINVAL);
	...
	wait(NULL);
	ASSERT_TEST(enable_policy(/*dead*/child_pid, 10, PASSWORD) == -1 && errno == ESRCH);
	return true;
...
bool test_disable_policy() {
	// test pid < 0
	ASSERT_TEST(disable_policy(-1, PASSWORD) == -1 && errno == ESRCH);
	// test (hopefully) pid does not exist
	ASSERT_TEST(disable_policy(INT_MAX -2, PASSWORD) == -1 && errno == ESRCH);
	// test policy already off
	ASSERT_TEST(disable_policy(getpid(), PASSWORD) == -1 && errno == EINVAL);
	// enabled policy
	enable_policy(getpid(), 10, PASSWORD);
	// test incorrect password
	...
	wait(NULL);
	// check success
	ASSERT_TEST(disable_policy(getpid(), PASSWORD) == 0);
	ASSERT_TEST(enable_policy(getpid(),10,PASSWORD) == 0);
	ASSERT_TEST(disable_policy(getpid(),PASSWORD) == 0);
	// test policy already off
	ASSERT_TEST(disable_policy(getpid(), PASSWORD) == -1 && errno == EINVAL);
...
bool test_set_process_capabilities() {
	...
	// test pid < 0
	ASSERT_TEST(set_process_capabilities(-1, 1, PASSWORD) == -1 && errno == ESRCH);
	// test (hopefully) pid does not exist
	ASSERT_TEST(set_process_capabilities(INT_MAX - 2, 1, PASSWORD) == -1 && errno == ESRCH);
	// test size new_level != 0, 1, 2
	ASSERT_TEST(set_process_capabilities(father_pid, -1, PASSWORD) == -1 && errno == EINVAL);
	ASSERT_TEST(set_process_capabilities(father_pid, 7, PASSWORD) == -1 && errno == EINVAL);
	// test incorrect password
	ASSERT_TEST(set_process_capabilities(father_pid, 1, 0) == -1 && errno == EINVAL);
	// test policy feature is off
	ASSERT_TEST(set_process_capabilities(father_pid, 1, PASSWORD) == -1 && errno == EINVAL);	
	// check success
	enable_policy(father_pid, 10, PASSWORD);
	int child_pid = fork();
	if(child_pid == 0){
		int i;
		for (i = 0; i <= 2; ++i) {
			ASSERT_TEST(set_process_capabilities(father_pid, i, PASSWORD) == 0);
		}
		exit(0);
	}
	wait(NULL);
...
bool test_get_process_log() {
	...
	enable_policy(getpid(), 2, PASSWORD);
...
bool test_all() {
	...
	if (child_pid == 0) {
		...
		ASSERT_TEST(disable_policy(getpid(), PASSWORD) == -1 && errno == EINVAL);
		set_process_capabilities(father_pid, 1, PASSWORD);
		exit(0);
	}
	...
	set_process_capabilities(father_pid, 2, PASSWORD);
	// privilege level = 2
	child_pid = fork();
	if (child_pid == 0) {
		set_process_capabilities(father_pid, 0, PASSWORD);
		exit(0);
	}
	sleep(2);
	// privilege = 0
	...
	disable_policy(father_pid, PASSWORD);
	// policy off
	ASSERT_TEST(wait(NULL) == child_pid);
	ASSERT_TEST(sched_yield() == 0);
	child_pid = fork();
	if (child_pid == 0) {
		// policy off for child
		ASSERT_TEST(disable_policy(getpid(), PASSWORD) == -1 && errno == EINVAL);
		enable_policy(father_pid, 5, PASSWORD);
		exit(0);
	}
	ASSERT_TEST(child_pid > 0);
	ASSERT_TEST(wait(NULL) == child_pid);
...
int main() {
	int test_child;
	RUN_TEST_CHILD(test_child, test_enable_policy);
	RUN_TEST_CHILD(test_child, test_disable_policy);
	RUN_TEST_CHILD(test_child, test_set_process_capabilities);
	RUN_TEST_CHILD(test_child, test_get_process_log);
	RUN_TEST_CHILD(test_child, test_all);
	return 0;
}
