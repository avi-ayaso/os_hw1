...
{
    if (_log_head == NULL || list_empty(_log_head)) return;
    int index = 0;
...
inline int copy_forbidden_activity_list_to_user(log_list * _log_head , forbidden_activity_info * ret_val , int size) {
	...
	ret_val = (forbidden_activity_info *) malloc(sizeof(forbidden_activity_info)*size);
	// problem with mem alloc.
	if (ret_val == NULL) return _EINVAL;
	list_for_each(iter,&(_log_head->_list_node)) {          //Traversing the list
		if (index == size) {
            break;
        }				// end of loop
        ...
        tmp = list_entry(iter,log_list,_list_node);  
        forbidden_activity_info * _tmp2 = &(tmp->forbidden_activity_info);
...
inline int add_forbidden_activity_to_log(log_list * _log_head , int _syscall_req_level , int _proc_level) {
    log_list * tmp = (log_list *) malloc (sizeof(log_list));
